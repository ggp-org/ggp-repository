...
<xsl:template name="main" match="/">
  <div>
    ...
    <style type="text/css" media="all">
      td.cell {
        width:  <xsl:value-of select="$width * 0.143"/>px;
        height: <xsl:value-of select="$height * 0.143"/>px;
      ...
      img.piece {
        width: <xsl:value-of select="$width * 0.9 * 0.143"/>px;
        height: <xsl:value-of select="$height * 0.9 * 0.143"/>px;        
      }
    </style>
